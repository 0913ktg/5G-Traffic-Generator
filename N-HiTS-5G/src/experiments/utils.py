...
from ..models.nhits_enhanced import NHITS

def normalize(x):
    """Normalize input in [-1,1] range, saving statics for denormalization"""
    max, min = x.max(), x.min()
    return (2 * (x - x.min())/(x.max() - x.min()) - 1), [max, min]
...
def instantiate_model(mc):
    MODEL_DICT = {'nhits': instantiate_nhits,}
...
# Cell
def model_train(mc, S_df, Y_df, X_df, f_cols, ds_in_val, ds_in_test, dataset_name, horizon, experiment_name, datatype):
