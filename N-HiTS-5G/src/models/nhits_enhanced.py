...
class _NHITSBlock(nn.Module):
    ...
    def __init__(self, n_time_in: int, n_time_out: int, n_x: int,
        ...
        assert (pooling_mode in ['cnn','max','average'])
        ...
        if pooling_mode == 'max':
            ...
            self.pooling_layer = nn.AvgPool1d(kernel_size=self.n_pool_kernel_size,
                                              stride=self.n_pool_kernel_size, ceil_mode=True)
        elif pooling_mode == 'cnn':
            if self.n_pool_kernel_size >= 4:
                self.padding_size = self.n_pool_kernel_size // 2 - 1
            self.pooling_layer = nn.Conv1d(in_channels=1, out_channels=1, 
                                  kernel_size=self.n_pool_kernel_size, stride=self.n_pool_kernel_size, padding=self.padding_size).cuda()
        ...
        # 1D CNN, GRU
        post_kenel_size = self.n_time_out//2
        post_hidden_size = 8
        self.h0 = torch.randn(1, 1, 8).cuda()
        self.post_1D_CNN = nn.Conv1d(in_channels=1, out_channels=1, kernel_size=post_kenel_size).cuda()
        self.post_GRU = nn.GRU(input_size = post_kenel_size + 1, hidden_size = post_hidden_size, num_layers = 1).cuda()
        self.post_lin = nn.Linear(in_features=post_hidden_size, out_features=n_time_out).cuda()
    ...
    def forward(self, insample_y: t.Tensor, insample_x_t: t.Tensor,
        ...
        # 1D CNN, GRU
        out = self.post_1D_CNN(forecast.unsqueeze(1))
        out, _ = self.post_GRU(out, self.h0)
        forecast = self.post_lin(out)
...
class _NHITS(nn.Module):
    ...
    def forecast(self, insample_y: t.Tensor, insample_x_t: t.Tensor, insample_mask: t.Tensor,
        ...
        for i, block in enumerate(self.blocks):
            ...
            if False:
                for_save = block_forecast.cpu().detach().numpy().flatten()
                np.save(f'stacks/stack{i+1}.npy', for_save)
...
class NHITS(pl.LightningModule):
    ...
    def training_step(self, batch, is_inf = False, batch_idx=0):
        S = batch['S']
        Y = batch['Y']
        X = batch['X']
        ...
        if is_inf:
            return loss, outsample_y, forecast
        else:
            return loss

    def validation_step(self, batch, idx=0):
        S = batch['S']
        Y = batch['Y']
        X = batch['X']
        sample_mask = batch['sample_mask']
